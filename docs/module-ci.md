...
To aggregate in this thesis repo, configure a `AGGREGATOR_TOKEN` secret in each module repo with permission to dispatch to `RonCatK/RonCatK-anthroDisturbance_Thesis`. The `Module CI Aggregator` workflow here listens for the `module-ci-report` dispatch and stores the payloads in `artifacts/module-reports/`.

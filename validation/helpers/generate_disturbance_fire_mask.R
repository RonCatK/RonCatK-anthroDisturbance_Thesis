...
maskDir <- parse_arg(
  "--mask-dir",
  Sys.getenv("UA_FIRE_MASK_DIR", unset = file.path(syntheticRoot, "fire"))
)
inputsPath <- parse_arg("--inputs-script", "")
...
if (nzchar(inputsPath)) {
  inputsPath <- normalizePath(inputsPath, winslash = "/", mustWork = FALSE)
  if (!file.exists(inputsPath)) {
    stop("Unable to locate inputs script at ", inputsPath, call. = FALSE)
  }
  source(inputsPath)
}
if (!exists("studyArea") || !exists("rasterToMatch")) {
  default_sa <- file.path(getwd(), "data", "study_area", "aoi_southwest_NWT.shp")
  if (!file.exists(default_sa)) {
    stop("Unable to build defaults; provide --inputs-script pointing to an R file that defines studyArea and rasterToMatch.", call. = FALSE)
  }
  studyArea <- terra::vect(default_sa)
